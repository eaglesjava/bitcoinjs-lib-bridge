...
class BILBTCWalletView: UIView, UITableViewDelegate, UITableViewDataSource {
	...
	func didSetWallet() {
		...
		w.getBalanceFromServer(success: { (wallet) in
			...
			self.currencyLabel.btcValue = NSDecimalNumber(string: w.btc_balanceString)
		}) { (msg, code) in
			debugPrint(msg)
		}
	}
    ...
    func loadTransactionHistory() {
        ...
        isLoading = true
        w.getTransactionHistoryFromSever(page: page, size: size, id: w.lastConfirmedTranscationServerID(), success: { (txs) in
